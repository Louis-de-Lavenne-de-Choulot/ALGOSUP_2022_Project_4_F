...
public class ToiletRush : ScenarioBase
{
    public override void StartScenario()
    {
        ...
        boundaries[1] = spots.FindIndex(boundaries[1] + 1,spots => spots.parent.name != "FToilets")-1;
        boundaries[2] = spots.FindIndex(boundaries[2] + 1,spots => spots.parent.name != "MToiletsWait") - 1;
    ...
    public override void GetNextGoal(NPC npc, int currentGoal)
    {
        ...
        switch (npc.name)
        {
            ...
            case "lindzy": //TODO : Add all femoïds
                for(int i = boundaries[3]; i <= boundaries[4];i++)
                {
                    if(bspots[i])
                    {
                        npc.ChangeGoal(spots[i]);
